...
class ConfigManager:
    ...
    def resolve_api_key(self, provider, session_key=""):
        ...
        # 1. SESSION (Typed) - Always takes priority so user can override ENV/Saved keys
        if session_key: return session_key, "Session Override"
        
        # 2. ENV VARS
        env_vars = {"google_gemini": ["GEMINI_API_KEY", "GOOGLE_API_KEY"], "openai": ["OPENAI_API_KEY"]}
...
def find_manual_proxy(source_path, custom_root=None):
    ...
    if custom_root and os.path.exists(custom_root):
        candidates.extend([os.path.join(custom_root, f"{name_no_ext}.mov"), os.path.join(custom_root, f"{name_no_ext}.mp4"), os.path.join(custom_root, filename), os.path.join(custom_root, "Proxy", f"{name_no_ext}.mov")])
    candidates.extend([os.path.join(folder, "Proxy", f"{name_no_ext}.mov"), os.path.join(folder, "Proxies", f"{name_no_ext}.mov")])
    for c in candidates:
...
def analyze_with_ai(frames, api_key, model, provider, filename):
    ...
    if "Gemini" in provider:
        ...
        messages = [{"role": "system", "content": "Cinematic editor."}, {"role": "user", "content": [{"type": "text", "text": f"Analyze: {filename}. {schema_prompt}"}]}]
...
def main():
    ...
    win = disp.AddWindow({'WindowTitle': 'Metafootage', 'ID': 'MainWin', 'Geometry': [400, 300, 620, 680]}, [
        ui.VGroup({'Spacing': 0}, [
            ...
            ui.VGroup({'ContentsMargins': [25, 0, 25, 10], 'Spacing': 12}, [
                ...
                ui.HGroup([
                    ...
                    ui.HGap(0, 1), 
                    ui.Label({'ID': 'KeySrc', 'Text': 'No Key', 'StyleSheet': 'color: #60a5fa; font-size: 9px; font-style: italic;'})
                ]),
                ui.HGroup([
                    ui.Label({'ID': 'KeyLabel', 'Text': 'API Key:', 'Weight': 0.3}), 
                    ui.HGroup({'Spacing': 4, 'Weight': 0.7}, [
                        ui.LineEdit({'ID': 'Key', 'EchoMode': 'Password', 'PlaceholderText': 'Paste key or use environment...'}), 
                        ui.Button({'ID': 'Tog', 'Text': 'Show', 'Checkable': True, 'Weight': 0, 'MinimumSize': [60, 28]})
                    ])
                ]),
                ui.HGroup([
                    ui.HGap(0, 0.3), 
                    ui.CheckBox({'ID': 'SaveKey', 'Text': 'Remember key on this machine', 'Checked': False, 'Weight': 1}), 
                    ui.Button({'ID': 'ClearKey', 'Text': 'Clear Saved', 'Weight': 0, 'StyleSheet': 'font-size: 9px; color: #f87171;'})
                ]),
                
                ui.VGap(10),
                ui.Label({'Text': 'RAW / PROXY SUPPORT', 'StyleSheet': 'color: #9ca3af; font-size: 10px; font-weight: bold;'}),
                ui.HGroup([ui.Label({'Text': 'Root Path:', 'Weight': 0.3}), ui.HGroup({'Spacing': 4, 'Weight': 0.7}, [ui.LineEdit({'ID': 'ProxyPath', 'Text': config.config.get('proxy_path', '')}), ui.Button({'ID': 'BrowseBtn', 'Text': 'Browse', 'Weight': 0, 'MinimumSize': [80, 28]})])]),
            ...
            ui.HGroup({'ContentsMargins': [25, 0, 25, 25], 'Spacing': 12, 'Weight': 0}, [
                ...
                ui.Button({'ID': 'Start', 'Text': 'Start Processing', 'MinimumSize': [170, 36], 'StyleSheet': 'background-color: #2563eb; color: white; font-weight: bold;'})
            ])
        ])
    ])
    ...
    def refresh_key_display(ev=None):
        ...
        typed = itm['Key'].Text
        key, source = config.resolve_api_key(p, typed)
        itm['KeySrc'].Text = f"Using: {source}"
        
        # Only clear/update models if the provider actually changed (to prevent model dropdown resets while typing)
        if ev and isinstance(ev, dict) and 'ID' in ev and ev['ID'] == 'Prov':
            itm['Mod'].Clear()
            if "Gemini" in p: itm['Mod'].AddItems(["gemini-3-flash-preview", "gemini-3-pro-preview"])
            else: itm['Mod'].AddItems(["gpt-4o", "gpt-4o-mini"])

    # Initial model load
    refresh_key_display({'ID': 'Prov'})

    # Events
    win.On.Prov.CurrentIndexChanged = refresh_key_display
    win.On.Key.TextChanged = refresh_key_display # CRITICAL: Update source label in real-time as user types
    ...
    def on_start(ev):
        ...
        run_data.update({'proceed': True, 'p': p, 'k': key, 'm': itm['Mod'].CurrentText, 'f': int(itm['Fr'].CurrentText), 'force': itm['Force'].Checked, 'proxy_root': itm['ProxyPath'].Text})
        disp.ExitLoop()
    ...
    # Processing Loop
    prog = disp.AddWindow({'WindowTitle': 'Processing...'}, [ui.VGroup({'ContentsMargins': [25, 20, 25, 25]}, [ui.Label({'ID': 'St', 'Alignment': {'AlignHCenter': True}}), ui.Label({'ID': 'Cn', 'Alignment': {'AlignHCenter': True}}), ui.Button({'ID': 'Stop', 'Text': 'Stop Analysis', 'StyleSheet': 'background: #f87171; color: white;'})])])
    p_itm, run_data['canceled'] = prog.GetItems(), False
    prog.On.Stop.Clicked = lambda ev: run_data.update({'canceled': True})
    prog.Show()

    for i, clip in enumerate(clips):
        if run_data['canceled']: break
        p_itm['Cn'].Text, p_itm['St'].Text = f"{i+1} / {len(clips)}", f"Analyzing: {clip.GetName()}"
        prog.RecalcLayout()
        
        path, _ = get_best_media_path(clip, run_data['proxy_root'])
        cache_key = config.get_cache_key(path, run_data['m'], run_data['f'])
        meta = config.cache.get(cache_key) if not run_data['force'] else None
        
        if not meta:
            try:
                frames = extract_frames(path, run_data['f'])
                meta, _ = analyze_with_ai(frames, run_data['k'], run_data['m'], run_data['p'], clip.GetName())
                if meta: config.cache[cache_key] = meta; config.save_cache()
            except: pass
            
        if meta:
            if isinstance(meta, list): meta = meta[0]
            existing_kw = [k.strip() for k in clip.GetMetadata("Keywords").split(",") if k.strip()]
            new_kw = [str(k).strip() for k in meta.get('keywords', [])]
            merged_kw = sorted(list(set(existing_kw + new_kw)))
            clip.SetMetadata({"Comments": meta.get('long_desc', ''), "Keywords": ", ".join(merged_kw), "Description": meta.get('short_desc', '')})
